    ...
    private function syncOrders(string $dateFrom, string $dateTo): void
{
    ...
    private function syncSales(string $dateFrom, string $dateTo): void
    {
        ...
        foreach ($rows as $row) {
            Sale::updateOrCreate(
                ['sale_id' => $row['saleID'] ?? ($row['sale_id'] ?? null)],
                $this->mapSale($row)
            );
        }
    ...
    private function syncIncomes(string $dateFrom, string $dateTo): void
    {
        ...
        foreach ($rows as $row) {
            Income::updateOrCreate(
                ['income_id' => $row['incomeId'] ?? ($row['income_id'] ?? null)],
                $this->mapIncome($row)
            );
        }
    ...
    private function syncReportDetail(string $dateFrom, string $dateTo): void
    {
        ...
        foreach ($rows as $row) {
            ReportDetail::updateOrCreate(
                ['rrd_id' => $row['rrdId'] ?? ($row['rrd_id'] ?? null)],
                $this->mapReportDetail($row)
            );
        }
